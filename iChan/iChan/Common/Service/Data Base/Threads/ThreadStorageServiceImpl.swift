...
class ThreadStorageServiceImpl: ThreadStorageService {
    ...
    func getAll() -> [Thread] {
        ...
        return crudService.get(type: ThreadModel.self, by: nil).map { (model: ThreadModel) -> Thread in
    ...
    func get(board: String, num: String) -> Thread? {
        ...
        if let found = crudService.get(type: ThreadModel.self, by: "\(board)-\(num)") {
    ...
    func delete(board: String, num: String) {
        ...
        let deleteCandidate = crudService.get(type: ThreadModel.self, by: NSPredicate(format: "pkey = '\(board)-\(num)'"))
        
        if let threadToDelete = deleteCandidate.first {
            
            crudService.delete(object: threadToDelete)
        }
    }
